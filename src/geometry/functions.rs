...
pub fn get_outer_and_inner_points(
    points: &HashSet<Point2D>, distance: u32
) -> (HashSet<Point2D>, HashSet<Point2D>) {
    let mut outer_points = get_outer_points(points);
    let mut visited = outer_points.clone();
    let mut queue = outer_points.iter().map(|p| (p.clone(), 0 as u32)).collect::<Vec<_>>();
    ...
    while queue.len() > 0 {
        ...
        for direction in CARDINALS_2D {
            ...
            outer_points.insert(neighbour.clone());
            queue.push((neighbour, edge_distance + 1));
        }
    }

    let inner_points = points - &outer_points;
...
pub fn is_straight_point2d(first: &Point2D, second: &Point2D, length: i32) -> bool {
    ...
    return ((line.x.abs() == length || line.y.abs() == length) && (line.x == 0 || line.y == 0)) || 
