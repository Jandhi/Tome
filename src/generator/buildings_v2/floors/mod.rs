...
impl FloorPlan {
    pub fn new(stairwells: Vec<Stairwell>) -> Self {
        ...
        let mut stair_tops: HashSet<(u32, i32, i32)> = stairwells.iter()
            ...
            .collect();
        // Also reserve the cell the player steps off onto when exiting the
        // stairwell at the top — one cell beyond the last step in the ascent
        // direction, on the upper floor.
        for sw in &stairwells {
...
impl Stairwell {
    ...
    /// The cell on the upper floor where the player steps off after climbing.
    /// One cell beyond the top step, in the direction from second-to-last
    /// to last position (the exit direction at the top of the stairwell).
