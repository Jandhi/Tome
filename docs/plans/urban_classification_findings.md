...
## Per-run results (city = count of urban super-districts)

| Run | #super-districts | min SD size | max SD size | urban SDs | city size | largest urban SD |
|---|---|---|---|---|---|---|
...
large neighbour — run 131607 absorbed a **35,034-cell** super-district that the old
...
(district→super-district, `merge.rs`) *and* `try_grow_city` (`classification.rs`). The fix changed
both. With adjacency now effective at 3× weight, merging is adjacency-dominated and runs
rich-get-richer: a large super-district has more border, so it attracts more merges.

### R2.3 District size is now wildly skewed — the headline
Every run produces super-districts spanning **~200 to 13,000–43,000 cells — a 60–200× range** (see
table). Consequence: **"city size = number of urban super-districts" is a meaningless metric.** A
...
1. **Re-express city size in cells/area, or bound super-district size in the merge.** This now outranks
   everything else. The 3–5 count target cannot yield consistent cities while a single super-district
   ranges 200–43,000 cells. Either measure the city by total cell count, or cap super-district size
...
2. **A/B the merge impact of the adjacency fix.** The size skew (R2.3) is plausibly amplified by
   adjacency now driving the merge (rich-get-richer). Compare super-district sizes pre/post-fix, and
...
(`merge_down` is now size-band driven — see `docs/plans/district_size_balancing.md`), so the merge
targets `S = interior_blocks / TARGET_DISTRICT_AMOUNT` and holds interior districts to
...
(Interior super-district count entering the merge was ~193–197 in every run; `S` is stable at ~3120–3160
...
Interior super-district spread collapsed from Round 2's **60–200×** to **max/min ≈ 2.0–2.9** in 9 of 10
runs (one outlier at 3.54). The ±50% band permits a max/min of 3.0, so the realised distribution is
essentially inside the target. Out-of-band districts total **11 across ~170** (~6%), most runs 0–2.
**Approach B resolves R2.3.**

### R3.2 Every band violation is the upper tail, and it's the circularity caveat
All 11 out-of-band districts are **above** the band (ratios 1.53–2.06); **none fall below the floor** —
...
(runs landed at 14–20), the realised avg drifts *below* `S` (e.g. 140943: 20 districts → avg 2432 vs
S 3153), so a district sitting near the absolute `U` reads as ratio > 1.5 against the lower realised avg.
...
### R3.3 One district above the absolute ceiling — the starvation relaxation, as designed
Run 141122's 5729-cell district exceeds even the absolute `U = 4692`. That's the intended
"merge-two-smalls / relax `U`" fallback firing when a below-L district had no in-cap parent — accepted
on purpose over stranding a district below the floor. Rare (1/10 runs); acceptable, but tightening the
...
side effect: evenly-sized interior super-districts mean more, contiguous Unknown growth candidates, so
...
1. **Close the upper-band gap (R3.2).** Make the band relative to the realised average: either re-derive
   ...
   out-of-band.
2. **Tighten the starvation fallback (R3.3)** so it can't leave a district well above `U`.
